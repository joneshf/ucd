...
while true; do
  ...
  if (( 1 <= num && num <= 14 )); then
    sed "s/PUT_THE_FILENAME_HERE/\"$(name $num)\"/" subtours_elimination_0_1.tmpl.zpl > subtours_elimination_0_1.zpl
    scip -f subtours_elimination_0_1.zpl > "$(tour $num)_elimination_0_1.log"
  elif [[ "$num" == q* ]]; then
    exit
  else
