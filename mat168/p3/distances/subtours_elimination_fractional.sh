...
while true; do
  ...
  if (( 1 <= num && num <= 14 )); then
    sed "s/PUT_THE_FILENAME_HERE/\"$(name $num)\"/" subtours_elimination_fractional.tmpl.zpl > subtours_elimination_fractional.zpl
    scip -f subtours_elimination_fractional.zpl > "$(tour $num)_elimination_fractional.log"
  elif [[ "$num" == q* ]]; then
    exit
  else
