...
to tree-growth
  ask patches [set living-neighbors count neighbors with [trees > 0] ];;N
  ask patches with [trees = 0] [;;bxy == 0 p = p* (N+1)/(k+1) 
    if (random 1001 / 1000) < growth-prob * ((living-neighbors + 1) / 9) [;;growth-prob = 0.05
      set trees 1
      set pcolor 60 + 5 * (trees / max-tree-growth)
    ]
  ]
  ask patches with [trees > 0 and trees < max-tree-growth] [;;bxy > 0
    set trees trees + .5 
...
to turtle-actions
  ask turtles [
    ...
    ifelse [trees] of patch-here > minimal-cut
...
to move-turtles
  let green-patches patches in-radius 2.83 with [trees > [minimal-cut] of myself]
